...
class GuidedCxtAtten(nn.Module):
    ...
    def forward(self, f, alpha, unknown=None, ksize=3, stride=1, fuse_k=3, softmax_scale=1., training=True):
        ...
        alpha_w = alpha_w.contiguous().view(
            raw_int_alpha[0], raw_int_alpha[2] // self.rate, raw_int_alpha[3] // self.rate, -1)
        alpha_w = alpha_w.contiguous().view(
            raw_int_alpha[0], -1, kernel, kernel, raw_int_alpha[1])
        ...
        w = w.contiguous().view(
            raw_int_fs[0], raw_int_fs[2] // self.rate, raw_int_fs[3] // self.rate, -1)
        ...
        if unknown is not None:
            ...
            assert unknown.size(2) == f.size(
                2), "mask should have same size as f at dim 2,3"
            unknown_mean = unknown.mean(dim=[2, 3])
            known_mean = 1 - unknown_mean
            unknown_scale = torch.clamp(torch.sqrt(
                unknown_mean / known_mean), 0.1, 10).to(alpha)
            known_scale = torch.clamp(torch.sqrt(
                known_mean / unknown_mean), 0.1, 10).to(alpha)
            softmax_scale = torch.cat([unknown_scale, known_scale], dim=1)
        else:
            unknown = torch.ones([fs[0], 1, fs[2], fs[3]]).to(alpha)
            softmax_scale = torch.FloatTensor([softmax_scale, softmax_scale]).view(
                1, 2).repeat(fs[0], 1).to(alpha)
        ...
        self_mask = self_mask.permute(2, 0, 1).view(
            1, fs[2] * fs[3], fs[2], fs[3]).float() * (-1e4)
        ...
        for xi, wi, alpha_wi, mmi, scale in zip(f_groups, w_groups, alpha_w_groups, mm_groups, scale_group):
            ...
            yi = yi * (scale[0, 0] * mmi.gt(0.).float() + scale[0, 1]
                       * mmi.le(0.).float())  # mmi => (1, 32*32, 1, 1)
            ...
            if self.rate == 1:
                ...
                yi = F.conv2d(yi, wi_center, padding=0) / \
                    4.  # (B=1, C=128, H=64, W=64)
            else:
        ...
        offsets = offsets - \
            torch.Tensor([fs[2]//2, fs[3]//2]).view(1,
                                                    2, 1, 1).to(alpha).long()
